...
export const updateAssistant = async (req, res) => {
    try {
        ...
        return res.status(500).json({
            message: "update assistant user error",
        });
...
export const askToAssistant = async (req, res) => {
    try {
        ...
        if (!req.userId) {
            return res.status(401).json({ response: "Unauthorized: userId missing" });
        }
        ...
        switch (type) {
            case 'get-date':
                return res.json({
                    ...
                    response: `current date ${moment().format("YYYY-MM-DD")}`
                })
            case 'get-time':
                return res.json({
                    type,
                    userInput: gemResult.userInput,
                    response: `current time ${moment().format("hh:mm A")}`
                })
            case 'get-day':
                return res.json({
                    type,
